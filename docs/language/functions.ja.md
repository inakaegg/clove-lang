...
### 1.4 クロージャのライフタイムに関する制限

`let`、`-defn`、`where` で直接束縛したクロージャが保持する呼び出し環境は解放されます。
一方、同じスコープのコレクション内へ間接的に格納したクロージャは、まだ循環参照を作る
場合があります。同じ環境を捕獲するクロージャを、その環境に束縛する vector や map の
中へ入れるような自己参照構造は避けてください。
